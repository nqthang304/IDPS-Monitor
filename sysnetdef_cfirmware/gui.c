...
void send_data_socket_uds_idps(const char *message_uds);
void get_current_timestamp(char *buf, size_t len);
void log_event_to_uds(const char* src_ip, const char* dst_ip, int src_port, int dst_port, 
                      const char* action, const char* rule_type, int severity, 
                      const char* protocol, int is_malware,
                      int packet_count, int packet_size);


#define IDPS_LOG_FILE "/home/antiddos/DDoS_V1/Log/Log_IDPS/idps.log"
#define SVR_COMMAND_SIZE 256
#define MAX_DEVICE_PORTS 4
#define MAX_DEVICE_PORTS_MIRORRING 3
/*===============================================*/
int luong = 0;
...
void sync_ipsec(sqlite3 *db, int profileId);
...
static void dongbo();
static void append_to_sync_buffer(char *buffer, size_t max_len, const char *id, int value);
//
...
// IPSEC
void *process_ipsec_thread_function(void *arg);
...
static time_t g_lcd_last_critical_time = 0;
//
...
volatile int g_restart_requested = 0;
volatile int g_snort_enabled = 0;
...
char buffer_send_fpga[2000] = "";
char buffer_send_gui[2000] = "";
////////////////////////////////////////////////////////////////////////////////
// Global Variables
I2C16x2 lcd;
...
pthread_mutex_t https_mutex = PTHREAD_MUTEX_INITIALIZER;
...
// char *socket_path_ipsec = "/tmp/ipsec";
int fd;           // TCP/IP socket
int fd_unix; // UNIX socket
char uds_msg[256];
char uds_msg_ipsec[256];

static char g_action[16] = "alert";
static char g_proto[8] = "tcp";
static char g_src_ip[32] = "any";
static char g_src_port[16] = "any";
static char g_dst_ip[32] = "any";
static char g_dst_port[16] = "any";

static char g_msg[128] = "";
static char g_content[128] = "";
static char g_direction[4] = "->";

static int g_sid = 1000001;
static int g_rev = 1;

static char g_mode[8] = "IDS";

// Global cho LCD IDPS
char g_idps_lcd_line1[17] = "IDPS: OFF       ";  
char g_idps_lcd_line2[17] = "                ";  
pthread_mutex_t idps_lcd_mutex = PTHREAD_MUTEX_INITIALIZER;
/*Function WOLFSSL*/
 ...
 * return interger [0,n]
 */
static unsigned long g_total_received = 0;
static unsigned long g_total_dropped  = 0;
static unsigned long g_total_passed   = 0;
pthread_mutex_t counter_mutex = PTHREAD_MUTEX_INITIALIZER;
static int intN(int n) { return rand() % n; }
// ============================================================
// Helper: setup bridge cho IDS mode
// ============================================================
static void setup_bridge(void)
{
    printf("[IDPS] Setting up bridge (IDS mode)...\n");
    fflush(stdout);

    system("sudo ip addr flush dev eth0");
    system("sudo ip addr flush dev eth1");

    system("sudo ip link add name br0 type bridge");
    system("sudo ip link set dev eth0 master br0");
    system("sudo ip link set dev eth1 master br0");

    system("sudo ip link set dev eth0 up");
    system("sudo ip link set dev eth1 up");
    system("sudo ip link set dev br0 up");

    printf("[IDPS] Bridge br0 ready.\n");
    fflush(stdout);
}

// ============================================================
// Helper: g? bridge khi không c?n (IPS mode ho?c t?t)
// ============================================================
static void teardown_bridge(void)
{
    printf("[IDPS] Tearing down bridge...\n");
    fflush(stdout);

    system("sudo ip link set dev br0 down");
    system("sudo ip link set dev eth0 nomaster");
    system("sudo ip link set dev eth1 nomaster");
    system("sudo ip link del br0");

    system("sudo ip link set dev eth0 up");
    system("sudo ip link set dev eth1 up");

    printf("[IDPS] Bridge removed.\n");
    fflush(stdout);
}
// ============================================================
// snort_restart() — CH? set flag + kick, KHÔNG pkill tr?c ti?p
// ============================================================
static void snort_restart(void)
{
    printf("[IDPS] Restart requested ? kicking idps_thread...\n");
    fflush(stdout);

    g_restart_requested = 1;

    // Kill và CH? h?n tru?c khi return
    system("sudo pkill -f snort");

    int waited = 0;
    while (system("pgrep -f snort > /dev/null 2>&1") == 0 && waited < 30)
    {
        usleep(100000);  // ch? t?i da 3 giây
        waited++;
    }

    printf("[IDPS] All Snort processes killed. Thread will restart.\n");
    fflush(stdout);
}

static void update_idps_lcd_mode(void)
{
    pthread_mutex_lock(&idps_lcd_mutex);
    if (!g_snort_enabled)
        snprintf(g_idps_lcd_line1, sizeof(g_idps_lcd_line1), "IDPS: OFF       ");
    else if (strcmp(g_mode, "IPS") == 0)
        snprintf(g_idps_lcd_line1, sizeof(g_idps_lcd_line1), "IDPS: IPS       ");
    else
        snprintf(g_idps_lcd_line1, sizeof(g_idps_lcd_line1), "IDPS: IDS       ");
    pthread_mutex_unlock(&idps_lcd_mutex);
}

static void split_ip_port(const char *full, char *ip_out, size_t ip_size, int *port_out)
{
    *port_out = 0;
    strncpy(ip_out, full, ip_size - 1);
    ip_out[ip_size - 1] = '\0';

    // -- N?u chu?i g?c ch?a '::' ? là IPv6, KHÔNG tách port --
    if (strstr(full, "::") != NULL) return;

    // -- T? dây ch? x? lý IPv4 d?ng "x.x.x.x:port" --
    char *last_colon = strrchr(full, ':');
    if (!last_colon) return;

    char *after = last_colon + 1;
    if (*after == '\0') return;

    // Ki?m tra ph?n sau ':' toàn là s?
    int all_digit = 1;
    ...
    // Tách IP + port
    *port_out = port;
    strncpy(ip_out, full, last_colon - full);
    ip_out[last_colon - full] = '\0';
}
// ============================================================
// idps_thread — CH? DUY NH?T kh?i d?ng Snort
// ============================================================
// Global — t?o 1 l?n khi program start
static char g_session_timestamp[32] = {0};

void init_idps_session_log(void)
{
    time_t now = time(NULL);
    struct tm *t = localtime(&now);
    strftime(g_session_timestamp, sizeof(g_session_timestamp), "%Y-%m-%d_%H-%M-%S", t);
    printf("[IDPS] Session log: %s\n", g_session_timestamp);
}
                      ...
                      int packet_count, int packet_size)
{
    ...
    snprintf(message, sizeof(message),
             "%s|%s|%s|%d|%d|%s|%s|%d|%s|%d|%d|%d\n",
             timestamp,
             (src_ip && strlen(src_ip) > 0) ? src_ip : "0.0.0.0",
             (dst_ip && strlen(dst_ip) > 0) ? dst_ip : "0.0.0.0",
             src_port, dst_port,
             action, rule_type, severity, protocol, is_malware,
             packet_count, packet_size);

    printf("[UDS-SEND] %s", message);
    fflush(stdout);
    send_data_socket_uds_idps(message);

    // -- Ch?n file log theo action + session timestamp --
    if (strcasecmp(action, "alert") == 0)
        snprintf(log_path, sizeof(log_path),
            "/home/antiddos/DDoS_V1/Log/Log_IDPS/idps_alert/%s.log",
            g_session_timestamp);
    else if (strcasecmp(action, "drop") == 0)
        snprintf(log_path, sizeof(log_path),
            "/home/antiddos/DDoS_V1/Log/Log_IDPS/idps_drop/%s.log",
            g_session_timestamp);
    else if (strcasecmp(action, "normal") == 0 || strcasecmp(action, "pass") == 0)
        snprintf(log_path, sizeof(log_path),
            "/home/antiddos/DDoS_V1/Log/Log_IDPS/idps_normal/%s.log",
            g_session_timestamp);
    else
        snprintf(log_path, sizeof(log_path),
            "/home/antiddos/DDoS_V1/Log/Log_IDPS/%s.log",
            g_session_timestamp);
...
void *idps_thread(void *arg)
{
    char cmd[512];
    char line[1024];

    printf("[IDPS] Thread started.\n");
    fflush(stdout);
    ...
    while (1)
    {
        ...
        if (strcmp(g_mode, "IPS") == 0)
        {
            teardown_bridge();
            snprintf(cmd, sizeof(cmd),
                "sudo snort -Q --daq afpacket -i eth0:eth1 -A alert_fast "
                "-c /usr/local/etc/snort/snort.lua 2>&1");
        }
        else
        {
            setup_bridge();
            snprintf(cmd, sizeof(cmd),
                "sudo snort --daq afpacket -i br0 -A alert_fast "
                "-c /usr/local/etc/snort/snort.lua 2>&1");
        }

        printf("[IDPS] Starting Snort (%s mode)...\n", g_mode);
        FILE *fp = popen(cmd, "r");
        if (fp == NULL) {
            perror("[IDPS] popen failed");
            sleep(2);
            continue;
        }
        ...
        {
            if (!g_snort_enabled || g_restart_requested) break;

            fflush(stdout);

            if (strstr(line, "Commencing packet processing"))
            {
                pthread_mutex_lock(&idps_lcd_mutex);
                snprintf(g_idps_lcd_line2, sizeof(g_idps_lcd_line2), "Status: NORMAL  ");
                pthread_mutex_unlock(&idps_lcd_mutex);
            }
            ...
            if (strstr(line, "[**]") != NULL)
            {
                ...
                char protocol[16]  = "TCP";
                char msg_name[256] = "Unknown Rule";

                char *msg_start = strchr(line, '"');
                char *msg_end   = msg_start ? strchr(msg_start + 1, '"') : NULL;
                if (msg_start && msg_end) {
                    size_t len = msg_end - msg_start - 1;
                    if (len > sizeof(msg_name) - 1) len = sizeof(msg_name) - 1;
                    strncpy(msg_name, msg_start + 1, len);
                    msg_name[len] = '\0';
                }

                if      (strstr(line, "{TCP}"))  strcpy(protocol, "TCP");
                else if (strstr(line, "{UDP}"))  strcpy(protocol, "UDP");
                else if (strstr(line, "{ICMP}")) strcpy(protocol, "ICMP");
                else                             strcpy(protocol, "IP");

                char *ip_part = strrchr(line, '}');
                if (ip_part) {
                    char src_full[128] = {0}, dst_full[128] = {0};
                    if (sscanf(ip_part + 1, " %127s -> %127s", src_full, dst_full) == 2) {
                        src_full[strcspn(src_full, "\r\n")] = '\0';
                        dst_full[strcspn(dst_full, "\r\n")] = '\0';
                        split_ip_port(src_full, src_ip, sizeof(src_ip), &src_port);
                        split_ip_port(dst_full, dst_ip, sizeof(dst_ip), &dst_port);
                    }
                }
                if (src_ip[0] == '\0') strcpy(src_ip, "0.0.0.0");
                if (dst_ip[0] == '\0') strcpy(dst_ip, "0.0.0.0");

                int is_normal_rule = (
                    strstr(msg_name, "Normal TCP Traffic")  != NULL ||
                    strstr(msg_name, "Normal UDP Traffic")  != NULL ||
                    strstr(msg_name, "Normal ICMP Traffic") != NULL ||
                    strstr(msg_name, "Normal IP Traffic")   != NULL
                );

                if (is_normal_rule)
                {
                    static unsigned int normal_batch_count = 0;
                    static time_t       last_normal_send   = 0;

                    normal_batch_count++;

                    pthread_mutex_lock(&idps_lcd_mutex);
                    if (time(NULL) - g_lcd_last_critical_time >= 3)
                        snprintf(g_idps_lcd_line2, sizeof(g_idps_lcd_line2), "Status: NORMAL  ");
                    pthread_mutex_unlock(&idps_lcd_mutex);

                    time_t now = time(NULL);
                    if (now - last_normal_send >= 2)
                    {
                        // -- THÊM: d?m normal --
                        g_total_received += normal_batch_count;
                        g_total_passed   += normal_batch_count;
                        // ----------------------
                        printf("[NORMAL] Sending batch: %u packets\n", normal_batch_count);
                        fflush(stdout);
                        log_event_to_uds(
                            src_ip, dst_ip,
                            src_port, dst_port,
                            "Normal", msg_name,
                            1, protocol,
                            0, normal_batch_count, 0
                        );
                        normal_batch_count = 0;
                        last_normal_send   = now;
                    }
                    continue;
                }

                const char *action = "Alert";
                int is_malware = 0;

                if (strcmp(g_mode, "IDS") == 0)
                {
                    if (strstr(line,     "trojan-activity") ||
                        strstr(msg_name, "TRITON")          ||
                        strstr(msg_name, "TRISIS")          ||
                        strstr(msg_name, "HATMAN")          ||
                        strstr(line,     "8517509"))
                    {
                        is_malware = 1;
                    }
                ...
                else
                {
                    ...
                    else if (strstr(line, "[Allow]") || strstr(line, "[pass]"))
                    {
                        action = "Normal";
                    }
                    else
                    {
                        if (strstr(line, "[drop]") || strstr(line, "[would_drop]"))
                            action = "Drop";
                        else if (strstr(line, "[alert]") || strstr(line, "[**]"))
                            action = "Alert";
                        else
                            action = "Drop";
                    }
                ...
                {
                    snprintf(g_idps_lcd_line2, sizeof(g_idps_lcd_line2), "Status: DROP    ");
                    g_lcd_last_critical_time = time(NULL);
                }
                else if (strcmp(action, "Alert") == 0)
                {
                    snprintf(g_idps_lcd_line2, sizeof(g_idps_lcd_line2), "Status: ALERT   ");
                    g_lcd_last_critical_time = time(NULL);
                }
                else if (time(NULL) - g_lcd_last_critical_time >= 3)
                {
                    snprintf(g_idps_lcd_line2, sizeof(g_idps_lcd_line2), "Status: NORMAL  ");
                }
                pthread_mutex_unlock(&idps_lcd_mutex);

                int severity = (strcmp(action, "Normal") == 0) ? 1 :
                               (is_malware || strcmp(action, "Drop") == 0) ? 3 : 2;

                // -- THÊM: d?m drop/pass --
                g_total_received++;
                if (strcmp(action, "Drop") == 0) g_total_dropped++;
                else                             g_total_passed++;
                printf("[STATS] recv=%-8lu drop=%-8lu pass=%-8lu\n",
                       g_total_received, g_total_dropped, g_total_passed);
                // -------------------------
        ...
        printf("  Total dropped  : %lu (%.2f%%)\n", g_total_dropped,
               g_total_received > 0 ? g_total_dropped * 100.0 / g_total_received : 0.0);
        printf("  Total passed   : %lu (%.2f%%)\n", g_total_passed,
               g_total_received > 0 ? g_total_passed  * 100.0 / g_total_received : 0.0);
        printf("=====================================\n");
        fflush(stdout);
        // ----------------------

        printf("[IDPS] Snort process exited.\n");
        fflush(stdout);

        system("sudo pkill -f snort > /dev/null 2>&1");
        int waited = 0;
        while (system("pgrep -f snort > /dev/null 2>&1") == 0 && waited < 30)
        {
            usleep(100000);
        ...
        {
            printf("[IDPS] Restart detected - restarting in %s mode...\n", g_mode);
            fflush(stdout);
            sleep(1);
            continue;
        }
...
void print_stacktrace()
{
  ...
  // Luu vào log file
  FILE *log = fopen("/var/log/ipsec_debug.log", "a");
  if (log)
  {
    time_t now = time(NULL);
    fprintf(log, "Stacktrace at %s:\n", ctime(&now));
    for (int i = 0; i < nptrs; i++)
    {
      fprintf(log, "%s\n", strings[i]);
    }
...
char socket_path_ipsec[128];
char UDS_DATA_PATH_IDPS[128];
int fd_idps = -1;

void load_socket_path() {
    FILE *f = fopen("/home/antiddos/DDoS_V1/socket.conf", "r");
    if (!f) {
        perror("open socket.conf");

        // fallback
        strcpy(socket_path, "/tmp/sysnetdef_data_antiddos");
        strcpy(socket_path_ipsec, "/tmp/sysnetdef_data_ipsec");
        strcpy(UDS_DATA_PATH_IDPS, "/tmp/sysnetdef_data_idps");
        return;
    }

    fgets(socket_path, sizeof(socket_path), f);
    fgets(socket_path_ipsec, sizeof(socket_path_ipsec), f);
    fgets(UDS_DATA_PATH_IDPS, sizeof(UDS_DATA_PATH_IDPS), f);
    fclose(f);

    // remove newline
    socket_path[strcspn(socket_path, "\n")] = 0;
    socket_path_ipsec[strcspn(socket_path_ipsec, "\n")] = 0;
    UDS_DATA_PATH_IDPS[strcspn(UDS_DATA_PATH_IDPS, "\n")] = 0;
}
void enqueue_idps_queue(unsigned char *buffer, int size) {
    pthread_mutex_lock(&idps_queue.mutex);

    int next_rear = (idps_queue.rear + 1) % PACKET_QUEUE_SIZE;

    // Ki?m tra hàng d?i d?y
    if (next_rear == idps_queue.front) {
        pthread_mutex_unlock(&idps_queue.mutex);
        return;
    }

    // Không dùng malloc n?a, copy tr?c ti?p vào m?ng tinh
    if (size > BUFFER_SIZE) size = BUFFER_SIZE; // Ð?m b?o không tràn buffer
    
    memcpy(idps_queue.packets[idps_queue.rear], buffer, size);
    idps_queue.sizes[idps_queue.rear] = size; // Luu kích thu?c vào m?ng sizes
    
    idps_queue.rear = next_rear;

    // Phát tín hi?u cho lu?ng IDPS
    pthread_cond_signal(&idps_queue.cond);
    pthread_mutex_unlock(&idps_queue.mutex);
}
...
static int search_whitelist(char key_src[50], char key_uri[200], int *ctr)
{
  ...
  else
  {
    ...
    {
      printf("[DEBUG] Checking WL: %s - %s\n", ptr_legit->ip_legit_client_addr, ptr_legit->uri_legit);
...
static void display_processing(void)
{
  ...
  struct processing_table *ptr_tmp = processing_table_0;

  if (ptr_tmp == NULL)
  {
    // printf("[DEBUG] Thread %ld: Processing table is empty\n", pthread_self());
    pthread_mutex_unlock(&table_mutex);
    return;
  }

  // printf("[DEBUG] Thread %ld: Current Processing Table:\n", pthread_self());
  int i = 0;
  while (ptr_tmp != NULL)
  {
    // printf("  Node %d:\n", i);
    // printf("    IP Client Addr : %s\n", ptr_tmp->ip_client_addr);
    // printf("    URI Request    : %s\n", ptr_tmp->uri_request_firt);
    // printf("    URI Redirect   : %s\n", ptr_tmp->uri_redirect);
  ...
  // printf("[DEBUG] End of Processing Table, total nodes: %d\n", i);

  pthread_mutex_unlock(&table_mutex);
}

static int search_processing_table(const char key_src[50], const char key_uri[200], int *ctr)
{
  int flag = 0;
  int i = 0;

  if (!key_src || !key_uri || !ctr)
    return -1;

  pthread_mutex_lock(&table_mutex);

  struct processing_table *ptr = processing_table_0;

  printf("[DEBUG] Searching processing table for IP='%s', URI='%s'\n", key_src, key_uri);

  while (ptr != NULL)
  {
    printf("[DEBUG] Node %d: IP='%s', URI_first='%s', URI_redirect='%s'\n",
           i, ptr->ip_client_addr, ptr->uri_request_firt, ptr->uri_redirect);

    if ((strlen(ptr->ip_client_addr) == strlen(key_src)) &&
        (memcmp(ptr->ip_client_addr, key_src, strlen(key_src)) == 0))
    {
      if ((strlen(ptr->uri_request_firt) == strlen(key_uri)) &&
          (memcmp(ptr->uri_request_firt, key_uri, strlen(key_uri)) == 0))
      {
        *ctr = i;
        flag = 0;
        printf("[DEBUG] Match found: Phase 1 (flag=0) at index %d\n", i);
        break;
      }
      if ((strlen(ptr->uri_redirect) == strlen(key_uri)) &&
          (memcmp(ptr->uri_redirect, key_uri, strlen(key_uri)) == 0))
      {
        *ctr = i;
        flag = 2;
        printf("[DEBUG] Match found: Phase 2 (flag=2) at index %d\n", i);
  ...
  if (ptr == NULL)
  {
    *ctr = -1;
    flag = 1;
    // printf("[DEBUG] No matching entry found (flag=1)\n");
  }
...
static void update_processing(char key_src[50], char key_uri_req_1[200], char uri_direct[200], int *ctr)
{
  ...
  else if (!nxt)
  {
    while (ptr_update != NULL)
    {
      ...
      // printf("\r\n");
      if ((memcmp(ptr_update->ip_client_addr, item_update_src, strlen(item_update_src)) == 0) &&
          (memcmp(ptr_update->uri_request_firt, item_update_uri_1, strlen(item_update_uri_1)) == 0))
      {
...
static void create_processing(char addr[50], char uri[200], char uri_direct[200])
{
  ...
  temp_processing = malloc(sizeof(struct processing_table));
  ...
  else
  {
    ...
    ptr_processing->next = temp_processing;
  }
  pthread_mutex_unlock(&table_mutex);
}
...
static void create_whitelist(char addr[50], char uri[200])
{
  ...
  temp = malloc(sizeof(struct whitelist_table));
  if (temp == NULL)
  {
    // printf("\r\nAlert: Out of Memory Space");
    // pthread_mutex_unlock(&table_mutex);
...
static void delete_processing(int pos)
{
  ...
  if (processing_table_0 == NULL)
  {
    // printf("[DEBUG] Processing table empty, nothing to delete\n");
    pthread_mutex_unlock(&table_mutex);
    return;
  }

  struct processing_table *ptr_tmp = processing_table_0;
  struct processing_table *prev = NULL;
  ...
  for (int i = 0; i < pos; i++)
  {
    ...
    if (ptr_tmp == NULL)
    {
      // printf("[DEBUG] Position %d not found in processing table\n", pos);
      pthread_mutex_unlock(&table_mutex);
      return;
    }
  }
  // printf("[DEBUG] Deleting node at pos %d: IP=%s, URI1=%s, URI_redirect=%s\n",
  //        pos, ptr_tmp->ip_client_addr, ptr_tmp->uri_request_firt, ptr_tmp->uri_redirect);

  if (prev == NULL)
  {
    processing_table_0 = ptr_tmp->next;
  }
  else
  {
    prev->next = ptr_tmp->next;
  }
...
// static const char *fixed_code_for_ip(const char *ip)
// {
//     if (strcmp(ip, "192.168.1.9") == 0)
//         return "chin";
...
void process_pkg_https(int ret, int shutDown, WOLFSSL *ssl, char *addr_client, int timer_pkg)
{
  huy1++;
  // printf("\n Huy1: %d\n", huy1);
  // pthread_mutex_lock(&https_mutex);
  /*Xu ly save white-black list*/
  char buff1[8192];
  char buff2[8192];
  char buff3[8192];
  char version_http[256];
  char version_http_1[256];
  char version_http_c[256];
  char version_http_c1[256];
  const char *header_down_responds = "Content-Length: 0\r\n"
                                     // "Connection: keep-alive\r\n\r\n";
                                     "Connection: close\r\n\r\n";
  const char *text_buff = " 302 Found\n"; // 301 Moved Permanently

  char URN[1024], URN1[1024], URN2[1024], URN_C[1024], URN1_C[1024];
  char URL[1024], URL1[1024], URL2[1024], URL_C[1024];
  char scheme[1024], new_uri[1024], save_uri[1024], save_uri1[1024];
  int a;
  int b;
  int c = 0;
  int d;
  ...
  char text_local[1024];
  char text_local1[1024];
  ...
  if (trusted_proceesing == 0)
  {
    ...
    // const char *URN_gen = fixed_code_for_ip(addr_client);
    // strcat(save_uri, "/");
    // strcat(save_uri, URN_gen);
  ...
  {
    // printf("\n==============A================");
    // printf("\n[PHASE I] First time request from client: %s", addr_client);
    // printf("\n[DEBUG] Client requested URI = '%s'", URN_C);

    if (strcmp(URN_C, "/") == 0)
    {
      // printf("\n[DEBUG] Client requested root URI '/' (no code)");
    }
    else
    {
      // printf("\n[DEBUG] Client requested custom URI '%s' (maybe manual or random)", URN_C);
    }
    ...
    strcat(save_uri, "/");
    strcat(save_uri, URN_gen);
    // printf("\n[DEBUG] Generated random code for redirect = '%s'", URN_gen);
    free(URN_gen);
    ...
    // printf("\n[DEBUG] Redirect Location header = '%s'", text_local);
    // printf("\n[DEBUG] HTTP response header to send:\n%s", version_http_c);
    ...
    len = strnlen(buff1, sizeof(buff1));
    // printf("\n[DEBUG] Final HTTP Response sent to client:\n%s", buff1);

    if ((ret = wolfSSL_write(ssl, buff1, len)) != (int)len)
    {
      // printf("\n[ERROR] wolfSSL_write failed when sending to client %s", addr_client);
    }
    else
    {
      // printf("[DEBUG] ok == 1 \n");
  ...
  else if (trusted_proceesing == 2)
  {
    ...
    pthread_mutex_lock(&table_mutex);
    struct processing_table *node = processing_table_0;
    int i = 0;
    while (node != NULL && i < c)
    {
      node = node->next;
    ...
    if (!node)
    {
      // printf("[DEBUG][%s] ERROR: Node not found at pos=%d\n", addr_client, c);
      return;
    }
    ...
    memset(text_local, 0, sizeof(text_local));
    strcat(text_local, "Location: ");
    strcat(text_local, node->uri_request_firt);

    memset(version_http_c, 0, sizeof(version_http_c));
    strcat(version_http_c, "HTTP/1.1 302 Found\n");
    strcat(version_http_c, text_local);
    // strcat(version_http_c, "\n\n");
    strcat(version_http_c, message_2);
    // printf("C:%s\n",version_http_c);

    // memset(text_local1, 0, sizeof(text_local1));
    // strcat(text_local1,text_local);
    // strcat(text_local1,"ABC");

    // memset(version_http_c1, 0, sizeof(version_http_c1));
    // strcat(version_http_c1, "HTTP/1.1 302 Found\n");
    // strcat(version_http_c1, text_local1);
    // // strcat(version_http_c1, "\n\n");
    // strcat(version_http_c1, message_2);
    // // printf("C1:%s\n",version_http_c1);

    len = strnlen(version_http_c, sizeof(version_http_c));

    // printf("Leng: %d\n",len);
    // printf("lengthucte:%d",strlen(version_http_c));
    // if ((ret = wolfSSL_write(ssl, version_http_c, len)) != (int)len) {
    //     printf("[DEBUG][%s] wolfSSL_write error (phase II)\n", addr_client);
    // } else {
    //     printf("[DEBUG][%s] Sending final redirect -> %s\n", addr_client, node->uri_request_firt);
    // }
    get_data_WL_from_Thread_HTTPS(addr_client);

    if ((ret = wolfSSL_write(ssl, version_http_c, len)) != (int)len)
    {
      // printf("[DEBUG][%s] wolfSSL_write error1 (phase II)\n", addr_client);
    }
    else
    {
      // printf("[DEBUG][%s] Sending final redirect1 -> %s\n", addr_client, node->uri_request_firt);
    }

    //   usleep(100000);
    //  //usleep(3000000);
    // int len1 = strnlen(version_http_c, sizeof(version_http_c));
    if ((ret = wolfSSL_write(ssl, version_http_c, len)) != (int)len)
    {
      // printf("[DEBUG][%s] wolfSSL_write error3 (phase II)\n", addr_client);
    }
    else
    {
      //  printf("[DEBUG][%s] Sending final redirect3 -> %s\n", addr_client, node->uri_request_firt);
    }
    // delete_processing(c);
    display_processing();

  } // pthread_mutex_unlock(&https_mutex);
}
...
void *process_https(void *args)
{
  ...
  //   if (!doPSK)
  //   {
  //     if (wolfSSL_CTX_use_certificate_buffer(ctx, server_cert_de r_2048,
  ...
  while (1)
  {
    ...
    if (clientfd == -1)
      ...
      {
        /* T?t TCP Keep-Alive trên socket */
        int optval = 0;
...
void connect_to_server()
{
  ...
  printf("\nConnected to UNIX server!\n");
}

// IPSEC
void connect_to_server_ipsec()
{
  struct sockaddr_un addr;

  if ((fd_unix = socket(AF_UNIX, SOCK_STREAM, 0)) == -1)
  {
    perror("\nsocket error\n");
  ...
  memset(&addr, 0, sizeof(addr));
  addr.sun_family = AF_UNIX;
  strncpy(addr.sun_path, socket_path_ipsec, sizeof(addr.sun_path) - 1);

  if (connect(fd_unix, (struct sockaddr *)&addr, sizeof(addr)) == -1)
  {
    perror("\nConnect error\n");
    close(fd_unix);
    return;
...
// IPSEC
void send_data_socket_uds_ipsec(const char *message_uds)
{
  if (fd_unix == -1)
  {
    connect_to_server_ipsec();
    if (fd_unix == -1)
      return;
  }
  if (write(fd_unix, message_uds, strlen(message_uds)) == -1)
  {
    //  printf("\nFD: %d\n", fd_unix);
    // perror("\nwrite error\n");
    close(fd_unix);
...
//IDPS
void connect_to_server_idps()
{
struct sockaddr_un addr;
    if ((fd_idps = socket(AF_UNIX, SOCK_STREAM, 0)) == -1) return;

    memset(&addr, 0, sizeof(addr));
    addr.sun_family = AF_UNIX;
    
    // Ép c?ng du?ng d?n kh?p v?i .env c?a Node.js
    strncpy(addr.sun_path, "/tmp/sysnetdef_data_idps", sizeof(addr.sun_path) - 1);

    if (connect(fd_idps, (struct sockaddr *)&addr, sizeof(addr)) == -1)
    {
        // Ð?ng perror liên t?c tránh tràn màn hình, ch? in nh? d? debug
        // perror("Connect IDPS error"); 
        close(fd_idps);
        fd_idps = -1;
        return;
    }
    printf("\n[SUCCESS] Connected to Node.js UDS Server!\n");
}
...
{
    if (fd_idps == -1) {
        connect_to_server_idps();
    }

    if (fd_idps != -1) {
        // Ð?i 'message' thành 'message_uds' d? kh?p v?i tham s? hàm
        ssize_t sent = send(fd_idps, message_uds, strlen(message_uds), MSG_NOSIGNAL);
        
        if (sent == -1) {
            // N?u g?i l?i (ví d? Node.js b? t?t), dóng socket d? l?n sau connect l?i
...
void process_buffer(char *client_message, int client_sock)
{
    ...
    memset(buffer_send_fpga, 0, sizeof(buffer_send_fpga));
    memset(buffer_send_gui, 0, sizeof(buffer_send_gui));
    ...
    while (token != NULL)
    {
        ...
        {
            printf("?? VALUE NULL for ID=%s\n", ID);
        }

        token = strtok(NULL, "$");
    }

    printf("===== PROCESS BUFFER END =====\n\n");
}
...
void process_key(int serial_port, char *ID, char *Value, int client_sock, char *buffer)
{
  ...
  //       strcat(buffer, Value);
  //       strcat(buffer, "$ERROR$");
  //       printf("\nADD SRC PORT %c MINOTORED error\n", ID[4]);
  //       break;
  //     }
  //     t++;
  //   }
  ...
  //       strcat(buffer, Value);
  //       strcat(buffer, "$OK$");
  //       printf("\nREMOVE SRC PORT %c MINOTORED done\n", ID[4]);
  //       break;
  //     }
  //     else if ((strchr(data1, 'N') != NULL) || (t == 10))
  //     {
  ...
  //       strcat(buffer, Value);
  //       strcat(buffer, "$ERROR$");
  //       printf("\nREMOVE SRC PORT %c MINOTORED error\n", ID[4]);
  //       break;
  //     }
  //     t++;
  //   }
  ...
  //       strcat(buffer, Value);
  //       strcat(buffer, "$ERROR$");
  //       printf("\nADD SRC IPV4 %c MINOTORED ERROR\n", ID[4]);
  //       break;
  //     }
  //     t++;
  //   }
  ...
  //       strcat(buffer, Value);
  //       strcat(buffer, "$OK$");
  //       printf("\nREMOVE SRC IPV4 %c MINOTORED done\n", ID[4]);
  //       break;
  //     }
  //     else if ((strchr(data1, 'N') != NULL) || (t == 10))
  //     {
  ...
  //       strcat(buffer, Value);
  //       strcat(buffer, "$ERROR$");
  //       printf("\nREMOVE SRC IPV4 %c MINOTORED ERROR\n", ID[4]);
  //       break;
  //     }
  //     t++;
  //   }
  ...
  //       strcat(buffer, Value);
  //       strcat(buffer, "$ERROR$");
  //       printf("\nADD DST IPV4 %c MINOTORED ERROR\n", ID[4]);
  //       break;
  //     }
  //     t++;
  //   }
  ...
  //       strcat(buffer, Value);
  //       strcat(buffer, "$OK$");
  //       printf("\nREMOVE DST IPV4 %c MINOTORED done\n", ID[4]);
  //       break;
  //     }
  //     else if ((strchr(data1, 'N') != NULL) || (t == 10))
  //     {
  ...
  //       strcat(buffer, Value);
  //       strcat(buffer, "$ERROR$");
  //       printf("\nREMOVE DST IPV4 %c MINOTORED ERROR\n", ID[4]);
  //       break;
  //     }
  //     t++;
  //   }
  ...
  //       strcat(buffer, Value);
  //       strcat(buffer, "$OK$");
  //       printf("\nADD PORT %c MINOTORED dst6 done\n", ID[4]);
  //       break;
  //     }
  //     else if ((strchr(data1, 'N') != NULL) || (t == 10))
  //     {
  ...
  //       strcat(buffer, Value);
  //       strcat(buffer, "$ERROR$");
  //       printf("\nREMOVE PORT %c MINOTORED  dst6 done\n", ID[4]);
  //       break;
  //     }
  //     t++;
  //   }
  ...
  // IPSec en/dis
  else if (strcmp(ID, "IPSEC_EN_DIS") == 0)
  {
    if (strcmp(Value, "1") == 0)
    {
      printf("\nStarting IPsec...\n");
      system("sudo /zynq_lib/sbin/ipsec start");
      usleep(10000);
      system("sudo /zynq_lib/sbin/ipsec up net12-net12");
    }
    else if (strcmp(Value, "0") == 0)
    {
      printf("\nStopping IPsec...\n");
      system("sudo /zynq_lib/sbin/ipsec stop");
    }
    strcat(buffer, ID);
    strcat(buffer, "$");
    strcat(buffer, Value);
  ...
  else if (strcmp(ID, "IPSEC_LOCAL_IP") == 0)
  {
    char command[256];
    sprintf(command, "sudo sed -i 's#^\\s*left=.*#        left=%s#' /zynq_lib/etc/ipsec.conf", Value);
    if (system(command) == -1)
    {
      perror("Error updating ipsec.conf");
    }

    printf("okay local_ip", Value);
  ...
  else if (strcmp(ID, "IPSEC_REMOTE_IP") == 0)
  {

    char command[256];
    sprintf(command, "sudo sed -i 's#^\\s*right=.*#        right=%s#' /zynq_lib/etc/ipsec.conf", Value);
    if (system(command) == -1)
    {
      perror("Error updating ipsec.conf");
    }
    // system("sudo sed -i 's#^\\s*right=.*#        right=%s#' /zynq_lib/etc/ipsec.conf",Value);
    printf("okay remote_ip ", Value);
  ...
  // IKEv2 Version
  else if (strcmp(ID, "IPSEC_IKE_VERSION") == 0)
  {
    if (strcmp(Value, "1") == 0)
      system("sudo sed -i 's#^\\s*keyexchange=ikev.*#        keyexchange=ikev1#' /zynq_lib/etc/ipsec.conf");
    else if (strcmp(Value, "2") == 0)
      system("sudo sed -i 's#^\\s*keyexchange=ikev.*#        keyexchange=ikev2#' /zynq_lib/etc/ipsec.conf");
  ...
  // // Mode
  else if (strcmp(ID, "IPSEC_MODE") == 0)
  {

    if (strcmp(Value, "transport") == 0)
    {
      system("sudo sed -i 's#type=[^ ]*#type=transport#' /zynq_lib/etc/ipsec.conf");
    }
    else if (strcmp(Value, "tunnel") == 0)
    {
      system("sudo sed -i 's#type=[^ ]*#type=tunnel#' /zynq_lib/etc/ipsec.conf");
    }
    strcat(buffer, ID);
    strcat(buffer, "$");
    strcat(buffer, Value);
  ...
  // // ESP/AH protocol
  else if (strcmp(ID, "IPSEC_PROTOCOL") == 0)
  {
    char command[256];
    if (strcmp(Value, "ESP") == 0)
    {
      sprintf(command, "sudo sed -i 's#^[[:space:]]*\\(esp\\|ah\\)=#        esp=#' /zynq_lib/etc/ipsec.conf");
    }
    else if (strcmp(Value, "AH") == 0)
    {
      sprintf(command, "sudo sed -i 's#^[[:space:]]*\\(esp\\|ah\\)=#        ah=#' /zynq_lib/etc/ipsec.conf");
    }
    if (system(command) == -1)
      perror("Error updating ipsec.conf");
  ...
  // else if (strcmp(ID, "IPSEC_ENCRY") == 0)
  // {
  //   char command[256];
  //   // C?p nh?t tr?c ti?p phía sau esp= ho?c ah=
  //   sprintf(command, "sudo sed -i 's#^\\([[:space:]]*\\(esp\\|ah\\)=\\).*#\\1%s-#' /zynq_lib/etc/ipsec.conf", Value);
  //   system(command);
  //   if (system(command) == -1)
  //   {
  //     perror("Error updating ipsec.conf");
  ...
  // // Hash Algorithm
  // else if (strcmp(ID, "IPSEC_HASH") == 0)
  // {
  //   char command[256];
  //   // Ghép ngay sau d?u '-' n?u dã có encryption, ho?c thêm m?c d?nh n?u chua
  //   sprintf(command,
  //           "sudo sed -i 's#^\\([[:space:]]*\\(esp\\|ah\\)=.*-\\).*#\\1%s#' /zynq_lib/etc/ipsec.conf", Value);
  //   system(command);
  //   if (system(command) == -1)
  //   {
  //     perror("Error updating ipsec.conf");
  ...
  {
    char command[256];
    char value_no_dash[64];

    // Lo?i b? d?u '-' tr?c ti?p
    int i, j = 0;
    for (i = 0; i < strlen(Value) && j < sizeof(value_no_dash) - 1; i++)
    {
      if (Value[i] != '-')
        value_no_dash[j++] = Value[i];
    }
    value_no_dash[j] = '\0';

    // C?p nh?t tr?c ti?p phía sau esp= ho?c ah=
    sprintf(command, "sudo sed -i 's#^\\([[:space:]]*\\(esp\\|ah\\)=\\).*#\\1%s-#' /zynq_lib/etc/ipsec.conf", value_no_dash);
    if (system(command) == -1)
      perror("Error updating ipsec.conf");
  ...
  else if (strcmp(ID, "IPSEC_HASH") == 0)
  {
    ...
    // Ghép ngay sau d?u '-' n?u dã có encryption
    sprintf(command,
            "sudo sed -i 's#^\\([[:space:]]*\\(esp\\|ah\\)=.*-\\).*#\\1%s#' /zynq_lib/etc/ipsec.conf", value_no_dash);
    if (system(command) == -1)
      perror("Error updating ipsec.conf");
  ...
  // IKE Reauth Time
  else if (strcmp(ID, "IPSEC_IKE_TIME") == 0)
  {
    char command[256];
    int seconds = atoi(Value);  // Value t? GUI là s? giây
    int minutes = seconds / 60; // chuy?n sang phút
    char iketime_str[32];
    sprintf(iketime_str, "%dm", minutes); // thêm ch? 'm'

    sprintf(command, "sudo sed -i 's#^\\s*ikelifetime=.*#        ikelifetime=%s#' /zynq_lib/etc/ipsec.conf", iketime_str);
    if (system(command) == -1)
    {
      perror("Error updating ipsec.conf");
    }
    ...
    strcat(buffer, ID);
    strcat(buffer, "$");
    strcat(buffer, iketime_str); // luu theo phút luôn
                                 //  strcat(buffer, "$OK$");
    printf("\nIPS_IKE_TIME done\n");
  }

  // Re-key Time
  else if (strcmp(ID, "IPSEC_KEY") == 0)
  {
    char command[256];
    int seconds = atoi(Value);
    int minutes = seconds / 60;
    char keylife_str[32];
    sprintf(keylife_str, "%dm", minutes);

    sprintf(command, "sudo sed -i 's#^\\s*keylife=.*#        keylife=%s#' /zynq_lib/etc/ipsec.conf", keylife_str);
    if (system(command) == -1)
    {
      perror("Error updating ipsec.conf");
    }
    ...
    strcat(buffer, ID);
    strcat(buffer, "$");
    strcat(buffer, keylife_str); // luu theo phút
                                 //  strcat(buffer, "$OK$");
    printf("\nIPS_KEY done\n");
  }
//=================================================================================================================
// IDPS
//=================================================================================================================
else if (strcmp(ID, "IDPS_EN_DIS") == 0)
{
    if (strcmp(Value, "1") == 0)
    {
        if (!g_snort_enabled)
        {
            printf("\n[IDPS] EN_DIS=1 ? will start Snort after DONE\n");
            g_snort_enabled = 1;
            g_pending_start = 1;
        }
        else
        {
            printf("\n[IDPS] EN_DIS=1 (already running) ? will restart after DONE\n");
            g_pending_start = 1;
        }
    }
    else
    {
        ...
        g_snort_enabled     = 0;
        g_pending_start     = 0;
        g_restart_requested = 0;  // reset, không cho thread popen l?i
        system("sudo pkill -f snort");
        update_idps_lcd_mode();  // kick fgets() thoát
        // idps_thread t? check g_snort_enabled=0 ? vào wait loop
    }
...
//=================================================================================================================
else if (strcmp(ID, "IDPS_MODE") == 0)
{
    snprintf(g_mode, sizeof(g_mode), "%s", Value);
    update_idps_lcd_mode();
    printf("\n[IDPS] Mode set to %s\n", g_mode);

    // KHÔNG restart ? dây — luôn ch? DONE
    // Tránh restart gi?a ch?ng khi GUI còn dang g?i các field khác
...
//=================================================================================================================
else if (strcmp(ID, "IDPS_UPDATE") == 0)
{
    printf("\n[IDPS] Updating rules ? Restarting Snort...\n");

    if (g_snort_enabled)
        snort_restart();
    else
        printf("[IDPS] Snort is disabled, skip restart.\n");
...
// else if (strcmp(ID, "IDPS_PROTOCOL") == 0)
// {
//     strcpy(g_proto, Value);
...
// else if (strcmp(ID, "IDPS_SRC_IP") == 0)
// {
//     strcpy(g_src_ip, Value);
...
// else if (strcmp(ID, "IDPS_DST_IP") == 0)
// {
//     strcpy(g_dst_ip, Value);
...
// else if (strcmp(ID, "IDPS_SRC_PORT") == 0)
// {
//     snprintf(g_src_port, sizeof(g_src_port), "%s", Value);
...
// else if (strcmp(ID, "IDPS_DST_PORT") == 0)
// {
//     snprintf(g_dst_port, sizeof(g_dst_port), "%s", Value);
...
// else if (strcmp(ID, "IDPS_DIRECTION") == 0)
// {
...
// else if (strcmp(ID, "IDPS_MSG") == 0)
// {
//     snprintf(g_msg, sizeof(g_msg), "%s", Value);
...
// else if (strcmp(ID, "IDPS_CONTENT") == 0)
// {
//     snprintf(g_content, sizeof(g_content), "%s", Value);
...
// else if (strcmp(ID, "IDPS_APPLY_RULE") == 0)
// {
//   printf(">>> HIT APPLY RULE <<<\n");
//     char rule[512];

//     sprintf(rule,
//         "%s %s %s %s %s %s %s (msg:\"%s\"; content:\"%s\"; sid:%d; rev:%d;)",
//         g_action,
//         g_proto,
//         g_src_ip,
//         g_src_port,
...
//     FILE *f = fopen("/usr/local/etc/snort/rules.txt", "a");
//     if (f)
//     {
//         fprintf(f, "%s\n", rule);
//         fclose(f);
...
//     if (strcmp(g_mode, "IPS") == 0)
//         system("sudo snort -c /usr/local/etc/snort/snort.lua -i eth0:eth1 -Q --daq afpacket > /dev/null 2>&1 &");
//     else
//         system("sudo snort -c /usr/local/etc/snort/snort.lua -i eth0 > /dev/null 2>&1 &");
...
#define DB_PATH "/home/antiddos/DDoS_V1/GUI/server/database/sysnetdef.db" // ÐU?NG D?N CHUNG

  // Hàm DUY NH?T: Ð?ng b? GUI v?i DB (web)
  void sync_ipsec(sqlite3 * db, int profileId)
  {
    int db_enable = 0;
    sqlite3_stmt *stmt;
    if (sqlite3_prepare_v2(db, "SELECT Enable FROM IPSecProfiles WHERE IPSecProfileId = ?;", -1, &stmt, 0) == SQLITE_OK)
    {
      sqlite3_bind_int(stmt, 1, profileId);

      if (sqlite3_step(stmt) == SQLITE_ROW)
        db_enable = sqlite3_column_int(stmt, 0);
      sqlite3_finalize(stmt);
    }
    int is_running = (system("ipsec status") == 0); // 0 = dang ch?y
    if (db_enable == 1 && !is_running)
    {
      system("sudo /zynq_lib/sbin/ipsec start");
      usleep(10000);
      system("sudo /zynq_lib/sbin/ipsec up net12-net12");
      printf("sys_ipsec start OK\n");
    }
    else if (db_enable == 0 && is_running)
    {
      system("sudo /zynq_lib/sbin/ipsec stop");
      // TODO: stop_ipsec(profileId);
      printf("sys_ipsec stop OK\n");
    }
  }
  // Hàm print_stacktrace c?i ti?n: in ra màn hình và luu vào log file

  // Encryption Algorithm trang 3 phase2
  // Encryption Algorithm

  //==================================================================================================================
  // IDS RULE En/Dis
  // else if (strcmp(ID, "IDS_EN_DIS") == 0)
  // {
  //   t = 0;
  //   write(serial_port, &key6, sizeof(key6));
  //   usleep(1000);
  ...
  // else if (strcmp(ID, "IPS_EN_DIS") == 0)
  // {
  //   t = 0;
  //   write(serial_port, &key6, sizeof(key6));
  //   usleep(1000);
  ...
  else if (strcmp(ID, "SNORT_PROTOCOL") == 0)
  {
    t = 0;
    // write(serial_port, &key6, sizeof(key6));
    // usleep(1000);
    ...
    while (1)
    {
      ...
      else if ((strchr(data1, 'N') != NULL) || (t == 10))
      {
        ...
        strcat(buffer, Value);
        strcat(buffer, "$ERROR$");
        printf("\nSNORT_PROTOCOL error\n");
        break;
      }
      t++;
    }
  }
  else if (strcmp(ID, "SNORT_SRC_IP") == 0)
  {
    t = 0;
    // write(serial_port, &key6, sizeof(key6));
    // usleep(1000);
  ...
  else if (strcmp(ID, "SNORT_SRC_PORT") == 0)
  {
    t = 0;
    // write(serial_port, &key6, sizeof(key6));
    // usleep(1000);
    ...
    while (1)
    {
      ...
      else if ((strchr(data1, 'N') != NULL) || (t == 10))
      {
        ...
        strcat(buffer, Value);
        strcat(buffer, "$ERROR$");
        printf("\nSNORT SRC PORT error\n");
        break;
      }
      t++;
    }
  }
  else if (strcmp(ID, "SNORT_DST_PORT") == 0)
  {
    t = 0;
    // write(serial_port, &key6, sizeof(key6));
    // usleep(1000);
...
// {
//     strcat(buffer_send_fpga, ID);
//     strcat(buffer_send_fpga, "$");
//     strcat(buffer_send_fpga, Value);
...
//     printf("\n\rBUFFER SEND UART:%s", buffer_send_fpga);
//     uart_send(buffer_send_fpga, serial_port);
...
// //         if ((strchr(data1, 'Y') != NULL))
// //         {
// //             strcat(buffer, buffer_send_fpga);
// //             strcat(buffer, "$OK$");
// //             break;
// //         }
// //     }
...
{
    // N?u dang ch? start Snort ? start t?i dây sau khi MODE dã du?c set
    if (g_pending_start)
    {
        printf("\n[IDPS] DONE ? Starting Snort in %s mode...\n", g_mode);
        snort_restart();
        g_pending_start = 0;
    }
...
// Send Snort to Uart
// void send_rule_uart(int fd, rule_t *rule)
// {
//   write(fd, rule, sizeof(rule_t));
//   tcdrain(fd);
// }
...
void *process_ipsec_thread_function(void *arg)
{
  ...
  while (1)
  {
    ...
    if (data_size > 0)
    {
      ...
      else if (eth->h_proto == htons(0x86DD))
      {
        protocol = payload[6];
      }
      int is_esp = (protocol == 50);
      int is_isakmp = 0;
      unsigned int udp_src_port = 0, udp_dst_port = 0;
      if (protocol == 17)
      {
        int udp_offset = (eth->h_proto == htons(0x0800)) ? (header_size + ((payload[0] & 0x0F) * 4)) : (header_size + 40);
        udp_src_port = (buffer[udp_offset] << 8) | buffer[udp_offset + 1];
        udp_dst_port = (buffer[udp_offset + 2] << 8) | buffer[udp_offset + 3];
        if (udp_src_port == 500 || udp_dst_port == 500)
          is_isakmp = 1;
      }
      if (is_esp || is_isakmp)
      {
        ...
        char src_ip_str[40] = "";
        char dst_ip_str[40] = "";
        snprintf(src_ip_str, sizeof(src_ip_str), "%u.%u.%u.%u", extracted_src_ip[12], extracted_src_ip[13], extracted_src_ip[14], extracted_src_ip[15]);
        snprintf(dst_ip_str, sizeof(dst_ip_str), "%u.%u.%u.%u", extracted_dst_ip[12], extracted_dst_ip[13], extracted_dst_ip[14], extracted_dst_ip[15]);
        ...
        // In ra t?ng dòng nhu Wireshark
        // Bóc tách IP/port dúng v? trí
        char ipv4_src[16] = "", ipv4_dst[16] = "";
        char ipv6_src[40] = "", ipv6_dst[40] = "";
        unsigned int udp_src_port = 0, udp_dst_port = 0;
        if (eth->h_proto == htons(0x0800))
        {
          // IPv4
          snprintf(ipv4_src, sizeof(ipv4_src), "%u.%u.%u.%u", payload[12], payload[13], payload[14], payload[15]);
          snprintf(ipv4_dst, sizeof(ipv4_dst), "%u.%u.%u.%u", payload[16], payload[17], payload[18], payload[19]);
          int ip_header_len = (payload[0] & 0x0F) * 4;
          if (protocol == 17)
          {
            udp_src_port = (payload[ip_header_len] << 8) | payload[ip_header_len + 1];
            udp_dst_port = (payload[ip_header_len + 2] << 8) | payload[ip_header_len + 3];
          }
        }
        else if (eth->h_proto == htons(0x86DD))
        {
          // IPv6
          snprintf(ipv6_src, sizeof(ipv6_src), "%02X%02X:%02X%02X:%02X%02X:%02X%02X:%02X%02X:%02X%02X:%02X%02X:%02X%02X",
          ...
          snprintf(ipv6_dst, sizeof(ipv6_dst), "%02X%02X:%02X%02X:%02X%02X:%02X%02X:%02X%02X:%02X%02X:%02X%02X:%02X%02X",
          ...
          if (protocol == 17)
          {
            udp_src_port = (payload[40] << 8) | payload[41];
            udp_dst_port = (payload[42] << 8) | payload[43];
          }
        }
        // G?p nhóm theo 5-tuple và tính BW, s? gói
        typedef struct
        {
          char src_ip[40];
          char dst_ip[40];
...
#define MAX_IPSEC_GROUPS 1024
        static ipsec_stat_t ipsec_stats[MAX_IPSEC_GROUPS];
        static int ipsec_stats_count = 0;
        static time_t last_print_time = 0;

        char group_src_ip[40] = "", group_dst_ip[40] = "";
        unsigned int group_src_port = 0, group_dst_port = 0;
        unsigned char group_protocol = protocol;
        int group_is_ipv6 = 0;
        if (is_esp && eth->h_proto == htons(0x0800))
        {
          snprintf(group_src_ip, sizeof(group_src_ip), "%s", ipv4_src);
          snprintf(group_dst_ip, sizeof(group_dst_ip), "%s", ipv4_dst);
          group_src_port = 0;
          group_dst_port = 0;
          group_is_ipv6 = 0;
        }
        else if (is_esp && eth->h_proto == htons(0x86DD))
        {
          snprintf(group_src_ip, sizeof(group_src_ip), "%s", ipv6_src);
          snprintf(group_dst_ip, sizeof(group_dst_ip), "%s", ipv6_dst);
          group_src_port = 0;
          group_dst_port = 0;
          group_is_ipv6 = 1;
        }
        else if (is_isakmp && eth->h_proto == htons(0x0800))
        {
          snprintf(group_src_ip, sizeof(group_src_ip), "%s", ipv4_src);
          snprintf(group_dst_ip, sizeof(group_dst_ip), "%s", ipv4_dst);
          group_src_port = udp_src_port;
          group_dst_port = udp_dst_port;
          group_is_ipv6 = 0;
        }
        else if (is_isakmp && eth->h_proto == htons(0x86DD))
        {
          snprintf(group_src_ip, sizeof(group_src_ip), "%s", ipv6_src);
          snprintf(group_dst_ip, sizeof(group_dst_ip), "%s", ipv6_dst);
          group_src_port = udp_src_port;
        ...
        if (is_esp || is_isakmp)
        {
          // Tìm nhóm
          int found = 0;
          for (int i = 0; i < ipsec_stats_count; ++i)
          {
            if (
                strcmp(ipsec_stats[i].src_ip, group_src_ip) == 0 &&
                strcmp(ipsec_stats[i].dst_ip, group_dst_ip) == 0 &&
                ipsec_stats[i].src_port == group_src_port &&
                ipsec_stats[i].dst_port == group_dst_port &&
                ipsec_stats[i].protocol == group_protocol &&
                ipsec_stats[i].is_ipv6 == group_is_ipv6)
            {
              ipsec_stats[i].bw += bw;
              ipsec_stats[i].pkt_counter += pkt_counter;
              found = 1;
              break;
            }
          }
          if (!found && ipsec_stats_count < MAX_IPSEC_GROUPS)
          {
            snprintf(ipsec_stats[ipsec_stats_count].src_ip, sizeof(ipsec_stats[ipsec_stats_count].src_ip), "%s", group_src_ip);
            snprintf(ipsec_stats[ipsec_stats_count].dst_ip, sizeof(ipsec_stats[ipsec_stats_count].dst_ip), "%s", group_dst_ip);
            ipsec_stats[ipsec_stats_count].src_port = group_src_port;
            ipsec_stats[ipsec_stats_count].dst_port = group_dst_port;
            ipsec_stats[ipsec_stats_count].protocol = group_protocol;
            ipsec_stats[ipsec_stats_count].bw = bw;
            ipsec_stats[ipsec_stats_count].pkt_counter = pkt_counter;
            ipsec_stats[ipsec_stats_count].is_ipv6 = group_is_ipv6;
            ipsec_stats_count++;
          }
        }
        ...
        {
          char print_time_str[32];
          time_t print_time = time(NULL);
          struct tm *print_tm = localtime(&print_time);
          strftime(print_time_str, sizeof(print_time_str), "%Y-%m-%d %H:%M:%S", print_tm);
          for (int i = 0; i < ipsec_stats_count; ++i)
          {
            if (ipsec_stats[i].is_ipv6)
            {
              if (ipsec_stats[i].src_port == 0 && ipsec_stats[i].dst_port == 0)
              {
                // printf("%s ESP: src_ip=%s src_port=N/A dst_ip=%s dst_port=N/A protocol=%u bw=%u pkt=%u\n",
                //   print_time_str, ipsec_stats[i].src_ip, ipsec_stats[i].dst_ip, ipsec_stats[i].protocol, ipsec_stats[i].bw, ipsec_stats[i].pkt_counter);

                snprintf(uds_msg_ipsec, sizeof(uds_msg_ipsec), "%s  %s  %s  %u  %u  %u  %u \n",
                         print_time_str, ipsec_stats[i].src_ip, ipsec_stats[i].dst_ip, ipsec_stats[i].src_port, ipsec_stats[i].dst_port, ipsec_stats[i].bw, ipsec_stats[i].pkt_counter);
                send_data_socket_uds_ipsec(uds_msg_ipsec);
                printf("%s  %s  %s  %u  %u  %u  %u ESP\n", print_time_str, ipsec_stats[i].src_ip, ipsec_stats[i].dst_ip, ipsec_stats[i].src_port, ipsec_stats[i].dst_port, ipsec_stats[i].bw, ipsec_stats[i].pkt_counter);
              }
              else
              {
                // printf("%s ISAKMP: src_ip=%s src_port=%u dst_ip=%s dst_port=%u protocol=%u bw=%u pkt=%u\n",
                //  print_time_str, ipsec_stats[i].src_ip, ipsec_stats[i].src_port, ipsec_stats[i].dst_ip, ipsec_stats[i].dst_port, ipsec_stats[i].protocol, ipsec_stats[i].bw, ipsec_stats[i].pkt_counter);
                snprintf(uds_msg_ipsec, sizeof(uds_msg_ipsec), "%s  %s  %s  %u  %u  %u  %u \n",
                         print_time_str, ipsec_stats[i].src_ip, ipsec_stats[i].dst_ip, ipsec_stats[i].src_port, ipsec_stats[i].dst_port, ipsec_stats[i].bw, ipsec_stats[i].pkt_counter);
                send_data_socket_uds_ipsec(uds_msg_ipsec);
                printf("%s  %s  %s  %u  %u  %u  %u ISAKMP\n", print_time_str, ipsec_stats[i].src_ip, ipsec_stats[i].dst_ip, ipsec_stats[i].src_port, ipsec_stats[i].dst_port, ipsec_stats[i].bw, ipsec_stats[i].pkt_counter);
              }
            }
            else
            {
    ...
    // if (current_log_file_normal != NULL)
    // {
    //   fprintf(current_log_file_normal, "%s %s %s %u %u %u %u %u %u %u\n",
    //           time_str, src_ip_str, dst_ip_str, src_port, dst_port, protocol, bw, pkt_counter, type, port_n);
    //   fflush(current_log_file_normal);
    // }
    // pthread_mutex_unlock(&log_mutex);
  }
...
void *lcd_thread_function(void *arg)
{
  while (1)
  {
    ...
    else if (g_snort_enabled)   
    {
      char lcd1[17], lcd2[17];

      pthread_mutex_lock(&idps_lcd_mutex);
      snprintf(lcd1, sizeof(lcd1), "%s", g_idps_lcd_line1);
      snprintf(lcd2, sizeof(lcd2), "%s", g_idps_lcd_line2);
      pthread_mutex_unlock(&idps_lcd_mutex);
...
void test_idps()
{
    ...
    process_key(serial_port, "IDPS_MODE", "IDS", client_sock, buffer);
    process_key(serial_port, "IDPS_PROTOCOL", "icmp", client_sock, buffer);
    process_key(serial_port, "IDPS_SRC_IP", "any", client_sock, buffer);
    process_key(serial_port, "IDPS_DST_IP", "any", client_sock, buffer);
    process_key(serial_port, "IDPS_DIRECTION", "inbound", client_sock, buffer);
    process_key(serial_port, "IDPS_MSG", "PING_TEST", client_sock, buffer);
    process_key(serial_port, "IDPS_CONTENT", "", client_sock, buffer);
    process_key(serial_port, "IDPS_APPLY_RULE", "1", client_sock, buffer);
}
void *memory_check_thread_function(void *arg)
{
...
void *run(void *arg)
{
  ...
  return_value = ChangeToWolfRoot();
    pthread_mutex_init(&idps_queue.mutex, NULL);
    pthread_cond_init(&idps_queue.cond, NULL);
    idps_queue.front = 0;
    idps_queue.rear = 0;
  ...
  // Create threads
      pthread_t idps_thread_id;
    if (pthread_create(&idps_thread_id, NULL, idps_thread, NULL) != 0) {
        perror("Không th? t?o lu?ng IDPS");
    }
  ...
  // IPSEC
  pthread_t ipsec_thread;
  pthread_create(&ipsec_thread, NULL, process_ipsec_thread_function, NULL);
  ...
  pthread_mutex_destroy(&https_mutex);
...
// //////////////////////////////////////////////Dongbo port by huy /////////////////////////////////////////////
// // Hàm d?ng b? hóa c?u hình cho M?T C?NG c? th?
static char *get_prefixed_id(const char *port_prefix, const char *suffix)
{
  static char id_buffer[64];
  snprintf(id_buffer, sizeof(id_buffer), "%s%s", port_prefix, suffix);
  return id_buffer;
}

// HÀM H? TR?: Thêm KEY$VALUE$ vào chu?i d?ng b?
static void append_to_sync_buffer(char *buffer, size_t max_len, const char *id, int value)
{
  ...
  size_t remaining_len = max_len - current_len;

  int written = snprintf(buffer + current_len, remaining_len, "%s$%d$", id, value);

  if (written < 0 || (size_t)written >= remaining_len)
  {
    fprintf(stderr, "Warning: Sync buffer overflow attempt. ID: %s\n", id);
  }
}

// // HÀM Ð?NG B? CHÍNH
// static void dongbo(int port)
// {
//     sqlite3 *db = NULL;
...
//     snprintf(port_prefix, sizeof(port_prefix), "PORT%d_", port);

//     // ==========================================================
//     // 2. TÌM PROFILE M?I NH?T CHO ÐÚNG PORT (eth1–eth4)
//     // ==========================================================

//     const char *sql =
//         "SELECT rowid, DefenseProfileUsingTime, DefenseProfileName "
...
//             if (!json_text || !profile_name) continue;
...
//                 const char *iface = cJSON_IsString(iface_obj) ? iface_obj->valuestring : NULL;
//                 const char *start = cJSON_IsString(start_obj) ? start_obj->valuestring : NULL;
...
//     if (latest_rowid == -1)
//     {
//         printf("Port %d: NO active profile found in DB.\n", port);
//         sqlite3_close(db);
//         return;
//     }

//     printf("Port %d (eth%d) using profile [%s] at time %s\n",
//             port, port, latest_profile_name, latest_time);

//     // ==========================================================
//     // 3. Ð?C C?U HÌNH T? PROFILE & T?O CHU?I SYNC
//     // ==========================================================
...
//     {
//         fprintf(stderr, "Query prepare error!\n");
//         sqlite3_close(db);
//         return;
//     }
...
//         // ====== Ð?c dúng 19 tru?ng ======
//         int syn_syn_thr = sqlite3_column_int(st, i++);
//         int syn_ack_thr = sqlite3_column_int(st, i++);
...
//         int tcpfrag_en  = sqlite3_column_int(st, i++);
//         int udpfrag_en  = sqlite3_column_int(st, i++);
...
//         // SYN
//         append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "SYN_EN_DIS"), syn_en);//ok
//         append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "SYN_THR"), syn_syn_thr);//ok
//         append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "ACK_THR"), syn_ack_thr);//ok
//         append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "TIME_WHITE_LIST"), wl_timeout);

//         // UDP
//         append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "UDP_EN_DIS"), udp_en);//ok
//         append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "UDP_THR"), udp_thr);//ok
//         append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "UDP_THR_PS"), udp_rate);//ok

//         // ICMP
//         append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "ICMP_EN_DIS"), icmp_en);//ok
//         append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "ICMP_THR"), icmp_thr);//ok
//         append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "ICMP_THR_PSS"), icmp_rate);//ok

//         // DNS
//         append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "DNS_EN_DIS"), dns_en);
//         append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "DNS_THR"), dns_thr);

//         // IPSEC IKE
//         append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "IPSEC_IKE_EN_DIS"), ike_en);
//         append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "IPSEC_IKE_THR"), ike_thr);

//         // Other
//         append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "LAND_EN_DIS"), land_en);
//         append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "TCP_FRAG_EN_DIS"), tcpfrag_en);
//         append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "UDP_FRA_EN_DIS"), udpfrag_en);
//         append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "HTTP_EN_DIS"), http_en);
//         append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "HTTPS_EN_DIS"), https_en);
//     }
...
//     if (strlen(sync_buffer) == 0)
//     {
//         printf("Port %d: sync_buffer EMPTY!\n", port);
//         return;
//     }

//     strcat(sync_buffer, "DONE$1");
//     strcpy(buffer_send_fpga, sync_buffer);

//     printf("\n\r==== Port %d: SENDING SYNC DATA TO UART ====\n", port);
//     printf("Buffer (%zu bytes):\n%s\n", strlen(buffer_send_fpga), buffer_send_fpga);

//     // ---- G?I PHÍM KÍCH HO?T C?U HÌNH (key1 + enter) ----
//     write(serial_port, &key1, sizeof(key1));
//     usleep(1000);
//     write(serial_port, &enter, sizeof(enter));
//     usleep(10000);

//     // ---- G?I N?I DUNG SYNC XU?NG FPGA ----
//     uart_send(buffer_send_fpga, serial_port);
...
//             if (strchr(data1, 'Y') != NULL)
//             {
//                 printf("FPGA ACCEPTED CONFIG (Y)\n");
//                 break;
//             }

//             if (strchr(data1, 'N') != NULL)
//             {
//                 printf("FPGA REJECTED CONFIG (N)\n");
//                 break;
//             }
//         }

//         usleep(100000);  // Ð?i 100ms r?i d?c l?i
//     }

//     // ---- CLEAR BUFFER (gi?ng process_key) ----
//     memset(buffer_send_fpga, 0, sizeof(buffer_send_fpga));
//     usleep(100000);

//     //printf("\n==== Port %d SYNC COMPLETED ====\n\n", port);
// }
...
static void dongbo(int port)
{
  ...
  snprintf(target_iface, sizeof(target_iface), "eth%d", port);
  snprintf(port_prefix, sizeof(port_prefix), "PORT%d_", port);

  // ----------------------------------------------------------
  // BU?C 2: TÌM PROFILE M?I NH?T
  // ----------------------------------------------------------
  ...
  if (sqlite3_prepare_v2(db, sql, -1, &stmt, NULL) == SQLITE_OK)
  {
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
      ...
      const char *profile_name = (const char *)sqlite3_column_text(stmt, 2);
      ...
      for (int i = 0; i < n; i++)
      {
        ...
        if (iface && start && strcmp(iface, target_iface) == 0)
        {
          if (latest_time[0] == 0 || strcmp(start, latest_time) > 0)
          {
            strncpy(latest_time, start, sizeof(latest_time) - 1);
            strncpy(latest_profile_name, profile_name, sizeof(latest_profile_name) - 1);
            latest_rowid = rowid;
          }
        }
      }
  ...
  if (latest_rowid == -1)
  {
    printf("Port %d: No active profile found.\n", port);
    sqlite3_close(db);
    return;
  }

  printf("\nPort %d (%s) - Profile: [%s] at %s\n",
         port, target_iface, latest_profile_name, latest_time);

  // ----------------------------------------------------------
  // BU?C 3: Ð?C C?U HÌNH T? DATABASE
  // ----------------------------------------------------------
  ...
  sqlite3_bind_int(stmt, 1, latest_rowid);

  if (sqlite3_step(stmt) == SQLITE_ROW)
  {
    int i = 0;
    int syn_thr = sqlite3_column_int(stmt, i++);
    int ack_thr = sqlite3_column_int(stmt, i++);
    int udp_thr = sqlite3_column_int(stmt, i++);
    int udp_rate = sqlite3_column_int(stmt, i++);
    int dns_thr = sqlite3_column_int(stmt, i++);
    int icmp_thr = sqlite3_column_int(stmt, i++);
    int icmp_rate = sqlite3_column_int(stmt, i++);
    int ipsec_thr = sqlite3_column_int(stmt, i++);
    int land_en = sqlite3_column_int(stmt, i++);
    int udp_en = sqlite3_column_int(stmt, i++);
    int dns_en = sqlite3_column_int(stmt, i++);
    int icmp_en = sqlite3_column_int(stmt, i++);
    int ipsec_en = sqlite3_column_int(stmt, i++);
    int tcp_frag_en = sqlite3_column_int(stmt, i++);
    int udp_frag_en = sqlite3_column_int(stmt, i++);
    int http_en = sqlite3_column_int(stmt, i++);
    int https_en = sqlite3_column_int(stmt, i++);
    int wl_timeout = sqlite3_column_int(stmt, i++);
    int syn_en = sqlite3_column_int(stmt, i++);

    // T?o chu?i sync buffer
    append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "SYN_EN_DIS"), syn_en);
    append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "SYN_THR"), syn_thr);
    append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "ACK_THR"), ack_thr);
    append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "TIME_WHITE_LIST"), wl_timeout);
    append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "UDP_EN_DIS"), udp_en);
    append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "UDP_THR"), udp_thr);
    append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "UDP_THR_PS"), udp_rate);
    append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "ICMP_EN_DIS"), icmp_en);
    append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "ICMP_THR"), icmp_thr);
    append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "ICMP_THR_PSS"), icmp_rate);
    append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "DNS_EN_DIS"), dns_en);
    append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "DNS_THR"), dns_thr);
    append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "IPSEC_IKE_EN_DIS"), ipsec_en);
    append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "IPSEC_IKE_THR"), ipsec_thr);
    append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "LAND_EN_DIS"), land_en);
    append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "TCP_FRAG_EN_DIS"), tcp_frag_en);
    append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "UDP_FRA_EN_DIS"), udp_frag_en);
    append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "HTTP_EN_DIS"), http_en);
    append_to_sync_buffer(sync_buffer, sizeof(sync_buffer), get_prefixed_id(port_prefix, "HTTPS_EN_DIS"), https_en);
  }
  ...
  // Thêm DONE$1 vào cu?i
  strcat(sync_buffer, "DONE$1");
  strcpy(buffer_send_fpga, sync_buffer);

  printf("\n==== Port %d: SENDING TO UART ====\n", port);
  ...
  // ----------------------------------------------------------
  // BU?C 5: Ð?C PH?N H?I T? FPGA
  // ----------------------------------------------------------

  int timeout_count = 0;
  const int MAX_TIMEOUT = 50; // 50 x 100ms = 5 giây
  ...
  while (timeout_count < MAX_TIMEOUT)
  {
    ...
    if (data1 != NULL && strlen(data1) > 0)
    {
      if (strstr(data1, "Port1:") || strstr(data1, "Port2:") ||
          strstr(data1, "Port3:") || strstr(data1, "Port4:") ||
          /*strstr(data1, "TFlood") ||*/ strstr(data1, "DefEN="))
      {
        // printf("[SKIP] Status report ignored\n");
        timeout_count++;
        usleep(100000);
        continue;
      }
      ...
      if (strlen(data1) <= 10)
      {
        if (strchr(data1, 'Y') != NULL)
        {
          // printf("? FPGA ACCEPTED CONFIG\n");
          break;
        }
        if (strchr(data1, 'N') != NULL)
        {
          // printf("? FPGA REJECTED CONFIG\n");
          break;
        }
      }
    }
  ...
  {
    printf("? TIMEOUT - No response from FPGA\n");
  }
  ...
  memset(buffer_send_fpga, 0, sizeof(buffer_send_fpga));
  usleep(200000);                  // 200ms delay tru?c port ti?p theo
  tcflush(serial_port, TCIOFLUSH); // Flush l?n cu?i
}
// ------------------------------------------------------------------------------
// HELPER FUNCTIONS CHO MIRROR
// ------------------------------------------------------------------------------

// Append string field: PORT3_MONITORED_DST_MAC$AA:BB:CC:DD:EE:FF$
static void append_mirror_str(char *buffer, size_t buf_size,
                              ...
                              const char *value)
{
  ...
  size_t current_len = strlen(buffer);
  size_t remaining = buf_size - current_len;

  snprintf(buffer + current_len, remaining,
           "%s%s$%s$", port_prefix, field_name, value);
}

// Append int field: PORT3_MONITORED$1$
static void append_mirror_int(char *buffer, size_t buf_size,
                              const char *port_prefix,
                              const char *field_name,
                              int value)
{
  ...
  snprintf(buffer + current_len, remaining,
           "%s%s$%d$", port_prefix, field_name, value);
}

// Append char field: PORT3_MIRRORING$E$
static void append_mirror_char(char *buffer, size_t buf_size,
                               const char *port_prefix,
                               const char *field_name,
                               char value)
{
  ...
  snprintf(buffer + current_len, remaining,
           "%s%s$%c$", port_prefix, field_name, value);
}

// ------------------------------------------------------------------------------
// HÀM Ð?NG B? PORT MIRRORING
// ------------------------------------------------------------------------------
// static void dongbo_mirror(int port)
// {
//     sqlite3 *db = NULL;
//     sqlite3_stmt *stmt = NULL;
...
//     // ---------------------------------------
//     // 2. L?y Mirror config t? DB
//     // ---------------------------------------
//     const char *query =
//         "SELECT InterfaceIsMonitoring, InterfaceIsMirroring, "
//         "       InterfaceMirrorSetting, MirrorType "
...
//     sqlite3_bind_text(stmt, 1, target_iface, -1, SQLITE_STATIC);

//     if (sqlite3_step(stmt) != SQLITE_ROW)
//     {
//         printf("Port %d: No mirror config found.\n", port);
//         sqlite3_finalize(stmt);
//         sqlite3_close(db);
//         return;
//     }

//     is_monitoring = sqlite3_column_int(stmt, 0);
//     int is_mirroring = sqlite3_column_int(stmt, 1);
//     const char *mirror_setting = (const char *)sqlite3_column_text(stmt, 2);
//     const char *mirror_type = (const char *)sqlite3_column_text(stmt, 3);

//     // MirrorType: ingress/egress/both -> I / E / B
//     if (mirror_type)
//     {
//         if (strcasecmp(mirror_type, "Ingress") == 0)
//             mirror_type_char = 'I';
//         else if (strcasecmp(mirror_type, "Egress") == 0)
//             mirror_type_char = 'E';
//         else if (strcasecmp(mirror_type, "Ingress and Egress") == 0)
//             mirror_type_char = 'IE';
//         else
//             mirror_type_char = 'N'; // None
//     }
// static void dongbo_mirror(int port)
// {
//     sqlite3 *db = NULL;
...
//     // Bi?n mirror
//     int is_monitoring = 0;
//     // S?A Ð?I: Kh?i t?o m?ng chu?i thay vì char don
//     char mirror_type_char[3] = ""; // I/E/B ho?c IE (d? ch? cho 2 ký t? + \0)
...
//     // MirrorType: Ingress/egress/both -> I / E / IE
//     if (mirror_type)
//     {
//         if (strcasecmp(mirror_type, "Ingress") == 0)
//             strcpy(mirror_type_char, "I");         // S?A: Gán chu?i
//         else if (strcasecmp(mirror_type, "Egress") == 0)
//             strcpy(mirror_type_char, "E");         // S?A: Gán chu?i
//         else if (strcasecmp(mirror_type, "Ingress and Egress") == 0)
//             strcpy(mirror_type_char, "IE");        // S?A: Gán chu?i
//         else
//             strcpy(mirror_type_char, "N");         // S?A: Gán chu?i
//     }

//     // printf("\n[MIRROR] Port %d (%s) - Monitoring=%d, Mirroring=%d, Type=%c\n",
//     //        port, target_iface, is_monitoring, is_mirroring, mirror_type_char);

//     // ---------------------------------------
//     // 3. Parse JSON InterfaceMirrorSetting
//     // ---------------------------------------
//     // if (mirror_setting && strlen(mirror_setting) > 0)
//     // {
//     //     cJSON *json = cJSON_Parse(mirror_setting);
//     //     if (json)
//     //     {
...
//     //             o = cJSON_GetObjectItem(val, "DestMac");
//     //             if (cJSON_IsString(o))
//     //                 strncpy(dst_mac, o->valuestring, sizeof(dst_mac) - 1);

//     //             o = cJSON_GetObjectItem(val, "SourceMac");
//     //             if (cJSON_IsString(o))
//     //                 strncpy(src_mac, o->valuestring, sizeof(src_mac) - 1);

//     //             o = cJSON_GetObjectItem(val, "DestIPv4");
//     //             if (cJSON_IsString(o))
//     //                 strncpy(dst_ipv4, o->valuestring, sizeof(dst_ipv4) - 1);

//     //             o = cJSON_GetObjectItem(val, "SourceIPv4");
//     //             if (cJSON_IsString(o))
//     //                 strncpy(src_ipv4, o->valuestring, sizeof(src_ipv4) - 1);

//     //             o = cJSON_GetObjectItem(val, "Protocol");
//     //             if (cJSON_IsString(o))
//     //                 strncpy(protocol, o->valuestring, sizeof(protocol) - 1);

//     //             cJSON *sp = cJSON_GetObjectItem(val, "SourcePort");
//     //             cJSON *dp = cJSON_GetObjectItem(val, "DestPort");

//     //             const char *spv = cJSON_IsString(sp) ? sp->valuestring : "";
//     //             const char *dpv = cJSON_IsString(dp) ? dp->valuestring : "";

//     //             if (strlen(spv) > 0 || strlen(dpv) > 0)
//     //                 snprintf(port_range, sizeof(port_range), "%s-%s", dpv, spv);
//     //         }
...
//         if (json)
//         {
//             // B? DÒNG: cJSON *val = cJSON_GetObjectItem(json, "Value");
//             // Thay b?ng vi?c ki?m tra tr?c ti?p json có ph?i là object không
//             if (cJSON_IsObject(json))
//             {
//                 cJSON *o;

//                 o = cJSON_GetObjectItem(json, "DestMac");
//                 if (cJSON_IsString(o))
//                     strncpy(dst_mac, o->valuestring, sizeof(dst_mac) - 1);

//                 o = cJSON_GetObjectItem(json, "SourceMac");
//                 if (cJSON_IsString(o))
//                     strncpy(src_mac, o->valuestring, sizeof(src_mac) - 1);

//                 o = cJSON_GetObjectItem(json, "DestIPv4");
//                 if (cJSON_IsString(o))
//                     strncpy(dst_ipv4, o->valuestring, sizeof(dst_ipv4) - 1);

//                 o = cJSON_GetObjectItem(json, "SourceIPv4");
//                 if (cJSON_IsString(o))
//                     strncpy(src_ipv4, o->valuestring, sizeof(src_ipv4) - 1);

//                 o = cJSON_GetObjectItem(json, "Protocol");
//                 if (cJSON_IsString(o))
//                     strncpy(protocol, o->valuestring, sizeof(protocol) - 1);

//                 cJSON *sp = cJSON_GetObjectItem(json, "SourcePort");
//                 cJSON *dp = cJSON_GetObjectItem(json, "DestPort");
...
//     // 4. T?o sync buffer
//     // ---------------------------------------
//     append_mirror_int(sync_buffer, sizeof(sync_buffer), port_prefix, "MONITORED", is_monitoring);
//     append_mirror_char(sync_buffer, sizeof(sync_buffer), port_prefix, "MIRRORING", mirror_type_char);

//     append_mirror_str(sync_buffer, sizeof(sync_buffer), port_prefix, "MONITORED_DST_MAC", dst_mac);
//     append_mirror_str(sync_buffer, sizeof(sync_buffer), port_prefix, "MONITORED_SRC_MAC", src_mac);
//     append_mirror_str(sync_buffer, sizeof(sync_buffer), port_prefix, "MONITORED_DST_IPV4", dst_ipv4);
//     append_mirror_str(sync_buffer, sizeof(sync_buffer), port_prefix, "MONITORED_SRC_IPV4", src_ipv4);
//     append_mirror_str(sync_buffer, sizeof(sync_buffer), port_prefix, "MONITORED_PORT", port_range);
//     append_mirror_str(sync_buffer, sizeof(sync_buffer), port_prefix, "MONITORED_PROTOCOL", protocol);

//     if (strlen(sync_buffer) == 0)
//     {
//         printf("Port %d: Mirror sync buffer EMPTY!\n", port);
//         return;
//     }
...
//     printf("\n==== Port %d: SENDING MIRROR CONFIG TO UART ====\n", port);
//     printf("Buffer (%zu bytes): %s\n", strlen(buffer_send_fpga), buffer_send_fpga);
...
//             if (strstr(data1, "OK"))
//             {
//                 printf("? FPGA ACCEPTED MIRROR CONFIG\n");
//                 break;
//             }
...
//                 if (strchr(data1, 'N'))
//                 {
//                     printf("? FPGA REJECTED MIRROR CONFIG\n");
//                     break;
//                 }
//             }
//         }
...
//     if (timeout_count >= MAX_TIMEOUT)
//         printf("? TIMEOUT - No response from FPGA (Mirror)\n");
...
static void dongbo_mirror(int port)
{
  ...
  // 2. L?y Mirror config t? DB
  // ---------------------------------------
  // ??? THÊM C?T Value VÀO QUERY ???
  const char *query =
      "SELECT InterfaceIsMonitoring, InterfaceIsMirroring, "
      "       InterfaceMirrorSetting, MirrorType, Value "
      "FROM DeviceInterfaces "
      "WHERE InterfaceName = ?";
  ...
  const char *mirror_setting = (const char *)sqlite3_column_text(stmt, 2);
  const char *mirror_type = (const char *)sqlite3_column_text(stmt, 3);
  const char *value_json = (const char *)sqlite3_column_text(stmt, 4); // ??? C?T Value ???

  // MirrorType: Ingress/Egress/Both -> I / E / IE
  if (mirror_type)
  {
    if (strcasecmp(mirror_type, "Ingress") == 0)
      strcpy(mirror_type_char, "I");
    else if (strcasecmp(mirror_type, "Egress") == 0)
      strcpy(mirror_type_char, "E");
    else if (strcasecmp(mirror_type, "Ingress and Egress") == 0)
      strcpy(mirror_type_char, "IE");
    else
      strcpy(mirror_type_char, "N");
  }
  else
  {
    strcpy(mirror_type_char, "N");
  ...
  // ---------------------------------------
  // 3. Parse JSON t? c?t Value
  // ---------------------------------------
  if (value_json && strlen(value_json) > 0)
  {
    cJSON *json = cJSON_Parse(value_json);
    if (json)
    {
      if (cJSON_IsObject(json))
      {
  ...
  // ---------------------------------------
  append_mirror_int(sync_buffer, sizeof(sync_buffer), port_prefix, "MONITORED", is_monitoring);
  append_mirror_str(sync_buffer, sizeof(sync_buffer), port_prefix, "MIRRORING", mirror_type_char);
...
void dongbo_all()
{
  for (int port_num = 1; port_num <= MAX_DEVICE_PORTS; port_num++)
  {
    dongbo(port_num);
  }
  for (int port_num = 1; port_num <= MAX_DEVICE_PORTS_MIRORRING; port_num++)
  {
    dongbo_mirror(port_num);
  }
}
...
int main()
{
  ...
  printf("[CONFIG] socket_path = %s\n", socket_path);
  printf("[CONFIG] socket_path_ipsec = %s\n", socket_path_ipsec);
  ...
  // L?p qua t?t c? các c?ng (t? 1 d?n MAX_DEVICE_PORTS) d? d?ng b? hóa

  // for (int port_num = 1; port_num <= MAX_DEVICE_PORTS; port_num++)
  // {
  //     // G?i hàm d?ng b? cho c?ng hi?n t?i
  //     dongbo(port_num);
  // }
   //test_idps();
  ModeGui(serial_port);
