...
export class IdpsRuleRequestService {
  ...
  /**
   * BULK IMPORT TỪ FILE .rules
   */
  async bulkImportFromRulesFile(fileContent: string) {
    const busy = HardwareBridge.checkSystemBusy();
    if (busy) this.throwBusyError(busy);

    if (!fileContent?.trim()) {
      throw new Error("The .rules file is empty.");
    }

    // 1. Parse file, bỏ qua dòng #
    const parsedRules = this.parseRulesFile(fileContent);
    if (parsedRules.length === 0) {
      throw new Error("No valid rules were found in the file (all lines are comments or incorrectly formatted).");
    }

    // 2. Lấy danh sách ruleId từ file
    const fileRuleIds = parsedRules.map(r => r.ruleId);

    // 3. Kiểm tra trùng lặp với database
    const existingIds = await this.idpsRuleRepository.getExistingRuleIds(fileRuleIds);
    if (existingIds.length > 0) {
      throw new Error(
        `Import rejected! Found ${existingIds.length} duplicate ruleID in the database: ${existingIds.join(', ')}. ` +
        `Do not import any rules to avoid conflicts.`
      );
    }

    // 4. Không trùng → tiến hành import tất cả
    await this.idpsRuleRepository.bulkCreateRules(parsedRules);

    // 5. Đồng bộ xuống phần cứng
    return await this.exportRulesToFiles();
  }

  /**
   * Parser file .rules - BỎ QUA dòng bắt đầu bằng #
   */
  private parseRulesFile(content: string): (typeof idpsRules.$inferInsert)[] {
    const lines = content.split('\n');
    const rules: (typeof idpsRules.$inferInsert)[] = [];
    ...
    for (const line of lines) {
      ...
      const rule = this.parseSingleRule(trimmed);
      if (rule) {
        rules.push(rule);
      }
    }
  ...
  /**
   * Parse một dòng rule hợp lệ
   */
  private parseSingleRule(line: string): (typeof idpsRules.$inferInsert) | null {
    const match = line.match(
      /^(alert|drop|pass|reject|sdrop|log)\s+(\w+)\s+(.+?)\s+->\s+(.+?)\s+\((.+)\)$/i
    );

    if (!match) return null;

    const [, actionRaw, protocolRaw, srcPart, dstPart, optionsStr] = match;
    const action = actionRaw.toLowerCase();
    const protocol = protocolRaw.toUpperCase();

    const options: Record<string, string> = {};
    const optionParts = optionsStr.split(';').map(p => p.trim()).filter(Boolean);
    ...
    const sidStr = options['sid'];
    if (!sidStr || isNaN(parseInt(sidStr))) return null;

    const ruleId = parseInt(sidStr);
    const description = options['msg']?.trim() || 'No description provided';

    const srcTokens = srcPart.trim().split(/\s+/);
    const dstTokens = dstPart.trim().split(/\s+/);

    const sourceIp = srcTokens[0] || 'Any';
    const sourcePort = srcTokens.length > 1 ? srcTokens[1] : 'Any';
    const destinationIp = dstTokens[0] || 'Any';
    const destinationPort = dstTokens.length > 1 ? dstTokens[1] : 'Any';

    let severity = 1;
    const classtype = options['classtype'] || '';
    if (/trojan|backdoor|malware/i.test(classtype)) severity = 3;
    else if (/attempted|recon/i.test(classtype)) severity = 4;
    else if (/policy|misc/i.test(classtype)) severity = 2;
